...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        for(int i=0; i < drywet.length; i++){
                if ((drywet[i][0] == longitude) && (drywet[i][1] == latitude)) {
                    for(int a = 2; a < 14; a++) drywetProbability [a - 2] = drywet[i][a];
                }
                if ((wetwet[i][0] == longitude) && (wetwet[i][1] == latitude)) {
                    for(int a = 2; a < 14; a++) wetwetProbability [a - 2] = wetwet[i][a];
                }
            }
        }
     ...
     * parameters:
     *      numberOfLocations:
     *          an integer representing how many lines exists in wetwet.txt and drywet.txt
     *      month: 
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        // COMPLETE THIS METHOD
        int longestWetSpell = 0;
        int longestDrySpell = 0;
        int wetSpell = 0;
        int drySpell = 0;
        for (int i = 0; i < forecast.length; i++) {
            if (forecast[i] == 1) {
